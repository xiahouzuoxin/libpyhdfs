...
static PyMethodDef HdfsMethods[] =
{
    ...
    {"read", hdfs_read, METH_VARARGS, "read(fs, hdfsfile[, size]) -> read at most min(128M, size) bytes, returned as a string \n\nIf the size argument is <=0 or omitted, read at most 128Mbytes. When EOF is reached, empty string will be returned"},
...
static struct PyModuleDef pyhdfs = {
